...
Mongoose is a compact multithreaded webserver. It

Table~\ref{t:mongoose_syscall} shows the system calls that are used by mongoose. In mongoose, both system calls are critical to the final output, any mismatch happens in those two system calls will either cause output divergence or different thread interleaving.
