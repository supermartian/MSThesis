...
\begin{figure}
\centering
\begin{lstlisting}[frame=tlrb]{blah}
void producer() {
	while (running){
		item = generate_item();
...
In some deterministic systems, they simply remove the tasks that in sleeping out of the deterministic schedule and put them back after they are back to user space. This is not applicable in a replication system like ours, as previously stated, the wake up time of those system calls might be different from the primary and secondary replica. As a result we must maintain those tasks sleeping in such system calls in the deterministic system, and also maintain the consistent state of the logical time for those tasks.
